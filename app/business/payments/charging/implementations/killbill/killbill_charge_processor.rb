...
class KillbillChargeProcessor
  ...
  def get_chargeable_for_params(params, _gumroad_guid)
    ...
    KillbillChargeablePaymentMethod.new(
      ...
      is_cryptocurrency: ActiveModel::Type::Boolean.new.cast(params[:is_cryptocurrency])
    )
  end
  ...
  def search_charge(purchase:)
    with_killbill_error_handler do
      ...
      payments = KillBillClient::Model::Payment.find_in_batches_by_search_key(
  ...
  def get_charge(charge_id, merchant_account: nil)
    with_killbill_error_handler do
      ...
      payment = KillBillClient::Model::Payment.find_by_id(charge_id, true, true, options)
  ...
  def get_charge_intent(payment_id, merchant_account: nil)
    with_killbill_error_handler do
      options = killbill_options(merchant_account)
      payment = KillBillClient::Model::Payment.find_by_id(payment_id, true, true, options)
  ...
  def setup_future_charges!(merchant_account, chargeable, mandate_options: nil)
    with_killbill_error_handler do
      ...
      payment_method = KillBillClient::Model::PaymentMethod.find_by_id(
  ...
  def create_payment_intent_or_charge!(merchant_account, chargeable, amount_cents, amount_for_gumroad_cents, reference,
                                       ...
                                       transfer_group: nil, off_session: true, setup_future_charges: false, mandate_options: nil, mandate_expected: false)
    with_killbill_error_handler do
      options = killbill_options(merchant_account)
      ...
      transaction.payment_external_key = transfer_group || reference
      transaction.properties = properties.map { |property| KillBillClient::Model::PluginPropertyAttributes.new(property) }

      created_payment = transaction.purchase(
        account_id,
        payment_method_id,
        options[:username],
        options[:reason],
  ...
  def confirm_payment_intent!(merchant_account, charge_intent_id)
    with_killbill_error_handler do
      options = killbill_options(merchant_account)
      payment = KillBillClient::Model::Payment.find_by_id(charge_intent_id, true, true, options)
  ...
  def cancel_payment_intent!(merchant_account, charge_intent_id)
    with_killbill_error_handler do
      ...
      void_transaction = KillBillClient::Model::Transaction.new
      void_transaction.payment_id = payment.payment_id
      void_transaction.transaction_type = "VOID"

      voided_payment = void_transaction.void(
        options[:username],
        options[:reason],
        options[:comment],
        options
      ...
      KillbillChargeIntent.new(payment: voided_payment, merchant_account:)
    end
  end
  ...
  def get_refund(refund_id, merchant_account: nil)
    with_killbill_error_handler do
      ...
      payment = KillBillClient::Model::Payment.find_by_transaction_id(refund_id, true, true, options)
  ...
  def holder_of_funds(merchant_account)
    if merchant_account && !merchant_account.is_managed_by_operator?
  ...
  private
    ...
    def refund_cryptocurrency_payment!(payment, amount_cents, options, is_for_fraud)
      ...
      credit_transaction = KillBillClient::Model::Transaction.new
      ...
      credit_transaction.properties = properties.map { |property| KillBillClient::Model::PluginPropertyAttributes.new(property) }
      updated_payment = credit_transaction.credit(
        payment.account_id,
      ...
      refund_transaction = updated_payment.transactions.reverse.find { |t| t.transaction_type == TRANSACTION_TYPE_CREDIT }

      KillbillChargeRefund.new(payment, refund_transaction, is_cryptocurrency: true)
    end
    ...
    def refund_fiat_payment!(payment, amount_cents, options, is_for_fraud)
      ...
      transaction = KillBillClient::Model::Transaction.new(
        ...
        transaction_external_key: "refund_#{payment.payment_id}_#{Time.now.to_i}",
        properties: properties
      )
      updated_payment = transaction.refund(
        options[:username],
        options[:reason],
        options[:comment],
        options
      ...
      refund_transaction = updated_payment.transactions.reverse.find { |t| t.transaction_type == TRANSACTION_TYPE_REFUND }
