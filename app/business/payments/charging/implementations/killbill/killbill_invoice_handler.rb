...
class KillbillInvoiceHandler
  ...
  def create_invoice_for_subscription(subscription_id, target_date: nil)
    with_killbill_error_handler do
      subscription = KillBillClient::Model::Subscription.find_by_external_key(
        subscription_id,
        killbill_options
      )
      ...
      invoice = KillBillClient::Model::Invoice.trigger_invoice(
  ...
  def add_credit_to_invoice(invoice_id, amount_cents, description: nil)
    with_killbill_error_handler do
      ...
      credit.amount = amount_cents / 100.0
  ...
  def get_invoice_payments(invoice_id)
    with_killbill_error_handler do
      payments = KillBillClient::Model::Invoice.new(invoice_id:).payments(true, true, "NONE", killbill_options)
  ...
  def retry_invoice_payment(invoice_id, payment_method_id: nil)
    with_killbill_error_handler do
      ...
      invoice_payment = KillBillClient::Model::InvoicePayment.new(
        ...
        payment_method_id:,
        purchased_amount: invoice.balance,
        currency: invoice.currency
      )
      payment = invoice_payment.create(
        false,
        killbill_options[:username],
        killbill_options[:reason],
        killbill_options[:comment],
  ...
  private
    ...
    def find_subscription_by_killbill_subscription_id(killbill_subscription_id)
      killbill_subscription = KillBillClient::Model::Subscription.find_by_id(
