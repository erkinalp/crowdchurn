...
class KillbillSubscriptionManager
  ...
  def create_subscription(subscription:, payment_method_id:, plan_name: nil)
    with_killbill_error_handler do
      ...
      killbill_subscription = KillBillClient::Model::Subscription.new
  ...
  def get_subscription(subscription_id)
    with_killbill_error_handler do
      killbill_subscription = KillBillClient::Model::Subscription.find_by_external_key(
        subscription_id,
        killbill_options
      )
  ...
  def pause_subscription(subscription_id)
    with_killbill_error_handler do
      ...
      killbill_subscription.set_blocking_state(
        "PAUSED", "crowdchurn-subscription", false, true, true, nil,
        killbill_options[:username],
        killbill_options[:reason],
        killbill_options[:comment],
        killbill_options
  ...
  def resume_subscription(subscription_id)
    with_killbill_error_handler do
      ...
      killbill_subscription.set_blocking_state(
        "ACTIVE", "crowdchurn-subscription", false, false, false, nil,
        killbill_options[:username],
        killbill_options[:reason],
        killbill_options[:comment],
        killbill_options
  ...
  def get_invoices(account_id)
    with_killbill_error_handler do
      invoices = KillBillClient::Model::Account.new(account_id:).invoices(killbill_options)
  ...
  def get_invoice(invoice_id)
    with_killbill_error_handler do
      invoice = KillBillClient::Model::Invoice.find_by_id(
        invoice_id,
        true,
        killbill_options
      )
  ...
  def pay_invoice(invoice_id, payment_method_id: nil)
    KillbillInvoiceHandler.new(@merchant_account).retry_invoice_payment(invoice_id, payment_method_id:)
  end

  def get_account_subscriptions(account_id)
    with_killbill_error_handler do
      bundles = KillBillClient::Model::Account.new(account_id:).bundles(killbill_options)
  ...
  private
    def configure_client
      ...
      @killbill_instance_url = instance_url
    end

    def killbill_options
      @killbill_options ||= {
        base_uri: @killbill_instance_url,
    ...
    def get_or_create_account(subscription)
      ...
      begin
        account = KillBillClient::Model::Account.find_by_external_key(
          external_key,
          false,
          false,
          killbill_options
