...
class KillbillPayoutProcessor
  ...
  private
    ...
    def perform_crypto_payout(payment, account_id, options)
      ...
      credit_transaction.payment_external_key = "crypto_payout_#{payment.external_id}"
      credit_transaction.properties = properties.map { |property| KillBillClient::Model::PluginPropertyAttributes.new(property) }
      created_payment = credit_transaction.credit(
        account_id,
        nil,
