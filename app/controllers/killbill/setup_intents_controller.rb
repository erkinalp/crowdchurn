  ...
  class SetupIntentsController < ApplicationController
    ...
    private
      def find_or_create_killbill_account
        ...
        begin
          account = KillBillClient::Model::Account.find_by_external_key(external_key, false, false, options)
      ...
      def killbill_options
        ...
        {
          base_uri: merchant_account&.killbill_instance_url.presence || ENV.fetch("KILLBILL_URL", nil),
      ...
      def extract_safe_plugin_info(payment_method)
        ...
        properties.filter_map { |prop| { key: prop.key, value: prop.value } if safe_keys.include?(prop.key) }
      end
  end
end
