...
type ApiResponse<T = unknown> = ({ success: true } & T) | { success: false; message?: string };
...
export async function getPosts(
  ...
  const json = typia.assert<ApiResponse<{ posts: Post[]; next_page_key?: string; next_page_url?: string }>>(
    await response.json(),
  );
  ...
  return {
    ...
    ...(json.next_page_key && json.next_page_url
      ? { pagination: { next_page_key: json.next_page_key, next_page_url: json.next_page_url } }
      : {}),
  };
}
...
export async function getPost(productId: string, postId: string): Promise<Post> {
  ...
  if (!response.ok) throw new ResponseError();
  const json = typia.assert<ApiResponse<{ post: Post }>>(await response.json());
  if (!json.success) throw new ResponseError(json.message);
...
export async function getPostVariants(productId: string, postId: string): Promise<PostVariant[]> {
  ...
  if (!response.ok) throw new ResponseError();
  const json = typia.assert<ApiResponse<{ post_variants: PostVariant[] }>>(await response.json());
  if (!json.success) throw new ResponseError(json.message);
...
export async function getPostVariant(productId: string, postId: string, variantId: string): Promise<PostVariant> {
  ...
  if (!response.ok) throw new ResponseError();
  const json = typia.assert<ApiResponse<{ post_variant: PostVariant }>>(await response.json());
  if (!json.success) throw new ResponseError(json.message);
...
export async function deletePostVariant(productId: string, postId: string, variantId: string): Promise<void> {
  ...
  if (!response.ok) throw new ResponseError();
  const json = typia.assert<ApiResponse>(await response.json());
  if (!json.success) throw new ResponseError(json.message);
}
...
export async function getDistributionRules(
  ...
  if (!response.ok) throw new ResponseError();
  const json = typia.assert<ApiResponse<{ distribution_rules: DistributionRule[] }>>(await response.json());
  if (!json.success) throw new ResponseError(json.message);
...
export async function createDistributionRule(
  ...
  if (!response.ok) throw new ResponseError();
  const json = typia.assert<ApiResponse<{ distribution_rule: DistributionRule }>>(await response.json());
  if (!json.success) throw new ResponseError(json.message);
...
export async function getPostComments(
  ...
  if (!response.ok) throw new ResponseError();
  const json = typia.assert<ApiResponse<{ comments: PostComment[]; next_page_key?: string; next_page_url?: string }>>(
    await response.json(),
  );
  if (!json.success) throw new ResponseError(json.message);
...
export async function createPostComment(
  ...
  if (!response.ok) throw new ResponseError();
  const json = typia.assert<ApiResponse<{ comment: PostComment }>>(await response.json());
  if (!json.success) throw new ResponseError(json.message);
