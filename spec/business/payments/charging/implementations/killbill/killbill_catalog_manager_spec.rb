...
describe KillbillCatalogManager do
  ...
  describe "#build_recurring_prices_for_currencies" do
    ...
    let(:price) { product.prices.find_by!(currency: "usd", recurrence: "monthly") }
    ...
    context "with JPY (single-unit currency)" do
      ...
      let(:price) { product.prices.find_by!(currency: "jpy", recurrence: "monthly") }
  ...
  describe "#generate_catalog_for_product" do
    ...
    context "with free trial" do
      let(:product) { create(:membership_product, :with_free_trial_enabled, user: seller, price_cents: 1000, price_currency_type: "usd", pricing_mode: :gross) }
