...
describe KillbillSubscriptionManager do
  ...
  let(:account_class) { class_double("KillBillClient::Model::Account").as_stubbed_const }
  ...
  describe "#resolve_account_currency" do
    ...
    context "when subscription has no billing_currency but link has price_currency_type" do
      ...
      let(:subscription) { build(:subscription, link: product, billing_currency: nil) }
  ...
  describe "#get_or_create_account" do
    ...
    before do
      allow(account_class).to receive(:find_by_external_key).and_raise(
        KillBillClient::API::NotFound.new(nil, instance_double(Net::HTTPResponse, code: "404", body: "Account not found"))
      )
    end
