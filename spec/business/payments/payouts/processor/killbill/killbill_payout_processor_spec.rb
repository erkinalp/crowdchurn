...
describe KillbillPayoutProcessor do
  ...
  describe ".is_balance_payable" do
    let(:merchant_account) { create(:merchant_account, user:, charge_processor_merchant_id: "balance-merchant") }
    ...
    context "when balance has no merchant account" do
      let(:balance) { build(:balance, user: user, merchant_account: nil) }
  ...
  describe ".prepare_payment_and_set_amount" do
    let(:balance_1) { create(:balance, user: user, merchant_account: killbill_merchant_account, amount_cents: 100_00, holding_amount_cents: 100_00) }
    let(:balance_2) { create(:balance, user: user, merchant_account: killbill_merchant_account, amount_cents: 200_00, holding_amount_cents: 200_00) }
  ...
  describe ".perform_payment" do
    ...
    let(:transaction) { KillBillClient::Model::Transaction.new }

    before do
      allow(KillBillClient::Model::Transaction).to receive(:new).and_return(transaction)
      allow(transaction).to receive(:credit).and_return(mock_payment_response)
    end
