...
describe Killbill::SetupIntentsController do
  ...
  let(:payment_method) do
    double(
      ...
      plugin_info: KillBillClient::Model::PaymentMethodPluginDetailAttributes.new(
        properties: [KillBillClient::Model::PluginPropertyAttributes.new(key: "last4", value: "4242")]
      )
    )
  end
  ...
  describe "POST create" do
    ...
    context "when Kill Bill account exists" do
      before do
        allow(KillBillClient::Model::Account).to receive(:find_by_external_key)
          .and_return(killbill_account)
        allow(KillBillClient::Model::PaymentMethod).to receive(:new)
    ...
    context "when Kill Bill account does not exist" do
      ...
      before do
        allow(KillBillClient::Model::Account).to receive(:find_by_external_key)
          .and_raise(KillBillClient::API::NotFound.new(nil, instance_double(Net::HTTPResponse, code: "404", body: "Account not found")))
        allow(KillBillClient::Model::Account).to receive(:new).and_return(new_account)
    ...
    context "when Kill Bill API returns bad request" do
      before do
        allow(KillBillClient::Model::Account).to receive(:find_by_external_key)
          .and_raise(KillBillClient::API::BadRequest.new(nil, instance_double(Net::HTTPResponse, code: "400", body: "Invalid request")))
      end
  ...
  describe "GET show" do
    ...
    context "when payment method exists" do
      before do
        allow(KillBillClient::Model::PaymentMethod).to receive(:find_by_id)
          .and_return(payment_method)
      end
      ...
      it "only returns safe plugin info properties" do
        crypto_payment_method = double(
          ...
          plugin_info: KillBillClient::Model::PaymentMethodPluginDetailAttributes.new(
            ...
            ].map { |property| KillBillClient::Model::PluginPropertyAttributes.new(property) }
          )
        )
        allow(KillBillClient::Model::PaymentMethod).to receive(:find_by_id)
    ...
    context "when payment method does not exist" do
      before do
        allow(KillBillClient::Model::PaymentMethod).to receive(:find_by_id)
          .and_raise(KillBillClient::API::NotFound.new(nil, instance_double(Net::HTTPResponse, code: "404", body: "Payment method not found")))
      end
