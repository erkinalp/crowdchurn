...
describe "Product::Searchable - Indexing scenarios" do
  ...
  describe "#as_indexed_json" do
    it "includes all properties" do
      ...
      @product.tag!("tag")
      @product.save_files!([{ external_id: SecureRandom.uuid, url: "#{S3_BASE_URL}/attachment/some-url.txt" }])
      @product.save_files!([
                             { external_id: SecureRandom.uuid, url: "#{S3_BASE_URL}/attachment/pencil.png" },
                             { external_id: SecureRandom.uuid, url: "#{S3_BASE_URL}/attachment/pic.jpg" },
                             { external_id: SecureRandom.uuid, url: "#{S3_BASE_URL}/attachment/other.jpg" },
                           ])
  ...
  describe "Indexing the changes through callbacks" do
    ...
    it "sends updated filetypes when a file is added" do
      expect_product_update %w[filetypes]
      @product.save_files!([{ external_id: SecureRandom.uuid, url: "#{S3_BASE_URL}/attachment/pic.jpg" }])
    end
