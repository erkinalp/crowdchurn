...
describe "S3Retrievable" do
  ...
  describe "#unique_url_identifier" do
    ...
    context "when it has an s3 guid" do
      before do
        s3_retrievable_object.url = "#{S3_BASE_URL}/attachments/23b2d41ac63a40b5afa1a99bf38a0982/original/nyt.pdf"
      end
  ...
  describe "#download_original" do
    ...
    it "raises a descriptive exception if the S3 object doesn't exist" do
      record = model.create!(url: "#{S3_BASE_URL}/attachments/missing.txt")
  ...
  describe "#restore_deleted_s3_object!" do
    context "when the versioned object exists" do
      let!(:record) { model.create!(url: "#{S3_BASE_URL}/#{SecureRandom.hex}") }
  ...
  describe "#confirm_s3_key!" do
    it "updates the url if possible" do
      ...
      record = model.create!(url: "#{S3_BASE_URL}/#{s3_directory}/incorrect-file-name.pdf")
  ...
  describe ".with_s3_key" do
    it "only includes s3 files matching the s3 key" do
      foo = model.create!(url: "#{S3_BASE_URL}/attachments/foo.pdf")
      foo2 = model.create!(url: "#{S3_BASE_URL}/attachments/foo.pdf")
      other = model.create!(url: "#{S3_BASE_URL}/attachments/other.pdf")
      model.create!(url: "https://example.com")
