...
describe SaveInstallmentService do
  ...
  let(:params) do
    ActionController::Parameters.new(
      installment: {
        ...
        files: [{ external_id: SecureRandom.uuid, stream_only: false, subtitles: [], url: "#{S3_BASE_URL}/attachment/some-url.txt" }],
  ...
  context "when installment is nil" do
    it "creates a product-type installment" do
      ...
      expect(installment.product_files.sole.url).to eq("#{S3_BASE_URL}/attachment/some-url.txt")
    end
  ...
  context "when installment is present" do
    ...
    it "marks the old file as deleted" do
      installment.product_files.create!(url: "#{S3_BASE_URL}/attachment/old-url.txt")
      service = described_class.new(seller:, installment:, params:, preview_email_recipient:)
      expect do
        service.process
      end.to change { installment.reload.product_files.count }.from(1).to(2)
      expect(installment.product_files.alive.sole.url).to eq("#{S3_BASE_URL}/attachment/some-url.txt")
      expect(service.error).to be_nil
    end
